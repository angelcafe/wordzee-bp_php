...
const VERSION       = '0.3.0';
const PE_PERMITIDOS = [ 'DP', 'TP', 'DL', 'TL', '' ];
$post_existe = true;
for( $x = 0; $x < 5; $x++ ) {
    for( $y = 0; $y < 3 + $x; $y++ ) {
        $tmp                      = 'pal' . ( $x + 3 ) . 'let' . ( $y + 1 );
...
$tpl_admin_css = '<link rel="stylesheet" href="front/indexbeta.css">';
...
$ronda1 = $ronda2 = $ronda3 = $ronda4 = $ronda5 = '';
...
$tpl_admin_js                    = '<script src="front/indexbeta.js"></script>';
...
function mostrarTpl( string $fichero, array $vars ) :array | string {
...
function palabrasEncontradas( array $palabras, array $puntos ) :string {
    $devolver = '';
    $max      = ( count( $palabras ) > 0 ) ? ( count( max( $palabras ) ) ) : ( 0 );
    for( $i = 0; $i < $max; $i++ ) {
        $devolver .= '<tr>';
        for( $x = 0; $x < 5; $x++ ) {
            $valor    = $palabras[ $x ][ $i ] ?? '';
    ...
    if( str_contains( $_SERVER["CONTENT_TYPE"], 'form' ) ) {
        echo $devolver;
        exit();
    }
